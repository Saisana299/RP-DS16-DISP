...
class UIUnison : public IUIHandler {
private:
...
public:
    ...
    void refreshUI() override {
        // タイトル
        pSprite->drawString("Unison Setting", 2, 2);
        ...
        pSprite->drawString("OSC2 Voice : " + String(o2v_chr) + "v", 2, 36);
        pSprite->drawString("OSC2 Detune: " + String(o2d_chr) + "%", 2, 46);

        // 塗り
        if(*displayCursor == 0x01) {
            cursorText("OSC1 Voice", 2, 16);
        }
        else if(*displayCursor == 0x02) {
            cursorText("OSC1 Detune", 2, 26);
        }
        else if(*displayCursor == 0x03) {
            cursorText("OSC2 Voice", 2, 36);
        }
        else if(*displayCursor == 0x04) {
            cursorText("OSC2 Detune", 2, 46);
        }
    }
